...
Provides fixtures and utilities for creating running containers with SSH access
...
SSH = NativeTool(
    name="ssh",
    package_hint="openssh-clients",
    reason="SSH into running test containers",
    when="dynamic-container-tests",
)

SSH_KEYGEN = NativeTool(
    name="ssh-keygen", 
    package_hint="openssh-clients",
    reason="generate SSH keys for container access",
    when="dynamic-container-tests",
)


class ContainerInstance(NamedTuple):
    """Running container instance with SSH access."""
    container_id: str
    container_name: str
    ip_address: str
    ssh_port: int
...
def generate_ssh_keypair(workdir: Path) -> tuple[Path, Path]:
    """Generate SSH keypair for container access."""
    key_dir = workdir / "ssh-keys"
    key_dir.mkdir(parents=True, exist_ok=True)
    
    private_key = key_dir / "test_key"
    public_key = key_dir / "test_key.pub"
    
    if not private_key.exists():
        logger.info("Generating SSH keypair for container access")
        _run_container_cmd([
            SSH_KEYGEN.name, "-t", "ed25519", 
            "-f", str(private_key),
            "-N", "",  # No passphrase
            "-C", "azl-container-test",
        ])
...
def create_container_with_exec(
    ...
    # Pre-install essential packages for better test compatibility
    logger.info("Installing essential packages for test compatibility")
    essential_packages = [
        "procps-ng",    # free, ps, uptime
        "util-linux",   # logger  
        "which",        # which command
        "hostname",     # hostname command
        "python3"       # python3 for tests
    ]
    
    # Try to install essential packages (don't fail if some don't install)
    install_result = exec_container_command_raw(
        container_name, 
        ["tdnf", "install", "-y"] + essential_packages, 
        timeout=60
    )
    ...
    logger.info("Container ready - additional packages will be installed on-demand for tests")
...
def exec_container_command_with_fallback(
    ...
    # If command failed with "command not found" and we have packages to try
    if (result.returncode == 127 and required_packages and 
...
def create_container_with_ssh(
    image_path: Path, 
    workdir: Path,
    container_name: str | None = None,
    image_name: str | None = None,
) -> ContainerInstance:
    """Create running container with SSH server enabled.
    
    Args:
        image_path: Path to image file or image reference
        workdir: Working directory for SSH keys
        container_name: Container name (auto-generated if None)
        image_name: Image name for logging (derived if None)
    """
    ...
    private_key, public_key = generate_ssh_keypair(workdir)
    ...
    # Create and start container with SSH setup
    run_cmd = [
        PODMAN.name, "run", "-d",
        "--name", container_name,
        "--replace",  # Replace existing container with same name
        "--publish", "22",  # Auto-assign SSH port from host
        "--tmpfs", "/run",
        "--tmpfs", "/tmp", 
        "--volume", "/sys/fs/cgroup:/sys/fs/cgroup:ro",
        "--env", f"SSH_PUBLIC_KEY={public_key_content}",
        "--cap-add", "SYS_ADMIN",  # Required for systemd
    ...
    logger.info("Waiting for container %s to start...", container_name)
    time.sleep(5)
    
    # Get container IP and SSH port
    inspect_result = _run_container_cmd([
        PODMAN.name, "inspect", container_name, "--format",
        "{{.NetworkSettings.IPAddress}}"
    ])
    ip_address = inspect_result.stdout.strip()
    
    # Get mapped SSH port
    port_result = _run_container_cmd([
        PODMAN.name, "port", container_name, "22"
    ])
    ssh_port = int(port_result.stdout.split(":")[-1])
    
    # Set up SSH access in container
    setup_ssh_in_container(container_name, public_key_content)
    
    # Test SSH connectivity
    test_ssh_access(ip_address, ssh_port, private_key)
    
    logger.info(
        "Container ready: %s (IP: %s, SSH: %d)", 
        container_name, ip_address, ssh_port
    )
...
def setup_ssh_in_container(container_name: str, public_key_content: str) -> None:
    """Set up SSH server and authorized keys in running container."""
    logger.info("Setting up SSH in container %s", container_name)
    ...
    setup_commands = [
        ...
        "chmod 700 /root/.ssh", 
        # Add public key
        f"echo '{public_key_content}' > /root/.ssh/authorized_keys",
        "chmod 600 /root/.ssh/authorized_keys",
        # Generate host keys if needed
        "ssh-keygen -A || true",
        # Start SSH service
        "systemctl enable --now sshd || /usr/sbin/sshd -D &",
    ]
    
    for cmd in setup_commands:
        exec_result = _run_container_cmd([
            PODMAN.name, "exec", container_name, "bash", "-c", cmd
        ])
        logger.debug("SSH setup command result: %s", exec_result.stdout[:100])


def test_ssh_access(ip_address: str, ssh_port: int, private_key: Path) -> None:
    """Test SSH connectivity to container."""
    logger.info("Testing SSH access to %s:%d", ip_address, ssh_port)
    ...
    for attempt in range(max_attempts):
        ...
        if attempt < max_attempts - 1:
            logger.debug("SSH not ready yet, attempt %d/%d", attempt + 1, max_attempts) 
            time.sleep(5)
    
    raise ContainerRuntimeError("SSH access test failed after all attempts")
...
def exec_ssh_command(
    ...
    """Execute command in container via SSH."""
    ssh_cmd = [
        SSH.name,
        "-i", str(container.ssh_key_path), 
        "-o", "StrictHostKeyChecking=no",
        "-o", "UserKnownHostsFile=/dev/null",
        "-o", "ConnectTimeout=10",
        "-p", str(container.ssh_port),
        f"{container.ssh_user}@{container.ip_address}",
        command
    ]
...
def destroy_container(container: ContainerInstance) -> None:
    """Stop and remove container."""
    logger.info("Destroying container %s", container.container_name)
