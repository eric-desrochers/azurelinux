...
def running_container(
    ...
    logger.info("Creating running container for dynamic testing")
...
# Optional SSH-based container for specific tests that need SSH networking
@pytest.fixture(scope="session")
def ssh_container(
    image_path: Path, image_type: str, image_name: str | None, workdir: Path, request: pytest.FixtureRequest
) -> ContainerInstance | None:
    """SSH-enabled container for tests that specifically require SSH networking.
    
    Use this fixture only for tests that need to test SSH functionality,
    network connectivity from external host, or other SSH-specific features.
    For most tests, use running_container (exec-based) instead.
    """
    if image_type != "container":
        pytest.skip("ssh_container only applicable to container images")
        
    # Check if any tests being run specifically request SSH
    has_ssh_tests = any(
        "ssh" in item.name.lower() or 
        (item.get_closest_marker("ssh_required") is not None)
        for item in request.session.items  
    ) if hasattr(request, 'session') else False
    
    if not has_ssh_tests:
        pytest.skip("ssh_container only for SSH-specific tests")
    
    logger.info("Creating SSH-enabled container for network testing")
    container = create_container_with_ssh(
        image_path, 
        workdir, 
        container_name=f"azl-ssh-{image_name or 'container'}",
        image_name=image_name
    )
    ...
    try:
        ...
        logger.info("Cleaning up SSH container")
        destroy_container(container)


@pytest.fixture
def ssh_exec_real(ssh_container: ContainerInstance):
    """Execute commands in SSH container via actual SSH (for SSH-specific tests)."""
    def _exec(command: str, timeout: int = 60) -> subprocess.CompletedProcess[str]:
        """Execute command via SSH and return result."""
        return exec_ssh_command(ssh_container, command, timeout)
    return _exec
