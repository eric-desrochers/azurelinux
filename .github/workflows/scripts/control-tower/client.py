...
Provides:
    ...
    * Job-status polling (long-form for prcheck, brief-form for build submit).
...
def post_scenario(
    ...
    *,
    ...
    ``base_url``. ``context`` is used in error messages to identify the call.
    """
    url = f"{base_url}{path}"
    ...
    if not response.ok:
        raise RuntimeError(
            f"Control Tower {context!r} request failed.\n" + format_error(response)
        )
    return _parse_json_object(response, f"Control Tower {context!r}")
